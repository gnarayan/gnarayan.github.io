...
The repository also contains `project/test_narayan_lab/`, an untracked work-in-progress Hugo source project for a lab group website using the `hugo-academic-group` theme.
...
### Lab website (`project/test_narayan_lab/`)

This is an independent Hugo site. Run commands from inside that directory:

```bash
cd project/test_narayan_lab

# Local development server with live reload
hugo server --watch

# Build static output to project/test_narayan_lab/public/
hugo
```

Content for the lab site lives in `project/test_narayan_lab/content/`. Theme is `hugo-academic-group` (in `themes/`). Configuration is in `config.toml`.
...
- **`project/test_narayan_lab/`**: Hugo source for a lab group site. Not yet deployed; uses lorem ipsum placeholder content. Theme from `themes/hugo-academic-group/`.
