...
The footer attribution line currently reads:
> Powered by the [Academic theme](https://sourcethemes.com/academic/) for [Hugo](https://gohugo.io), [Claude Code](https://claude.ai/code), and [vim](https://www.vim.org).

Both files must be kept in sync if this line changes.
...
- **`img/logos/`**: All institution and project logos. Footer bar logos use `logo-*.webp` (or `.svg`). Experience card logos use `exp-*.webp` (or `.svg`). All PNGs have WebP counterparts; reference the `.webp` in HTML.
- **`img/logos/logo-scimma.webp`**: 180×180 WebP of the SCiMMA ♋ logo in Rubin green (`#00B68C`). The original source is the SCiMMA raster PNG at `https://scimma.org/static/imgs/logos/logo_transparent.png`. The current file is a clean vector reconstruction: the original was pixel-analysed to extract circle positions and arc geometry, which were re-expressed as SVG strokes with analytically computed endpoints (circle–circle intersection) to ensure perfectly circular hole interiors. The standalone project lives in `~/Dropbox/work/scimma_logo/` and contains the SVG, high-res PNG, lockup assets, and a `CLAUDE.md` with the full geometry and a self-contained regeneration script. Do not regenerate `logo-scimma.webp` in this repo — update the Dropbox project and copy the result here.
